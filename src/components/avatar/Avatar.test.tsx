...
import Avatar, {TestIDs} from "./Avatar";
...
const getIcon = () => get.Root().querySelector("img[src]");
...
describe("Avatar", () => {
    ...
    describe("icon", () => {
        ...
        it("can be given size", () => {
            const size = 123;
            render({ size });
            const iconParent = get.Root().querySelector("div[style*=width][style*=height]");
            expect(iconParent).toHaveStyle(`width: ${size}px`);
            expect(iconParent).toHaveStyle(`height: ${size}px`);
        });
    });
